...
const TYPE24 = 'scaled24h';
const TYPE36 = 'scaled36h';
const TYPE48 = 'scaled48h';
const TYPES = [TYPE24, TYPE36, TYPE48];

// logoType property supports only 3 types at this point
const isValidLogoType = mode => TYPES.includes(mode);
...
const getHeightClassName = logoType => {
  return logoType === TYPE48 ? css.logo48 : logoType === TYPE36 ? css.logo36 : css.logo24;
};
...
export const LogoComponent = props => {
  ...
  const hasValidLogoType = isValidLogoType(logoType);
  const logoClasses = className || css.root;
  const logoImageClasses = classNames(logoImageClassName || css.logo, getHeightClassName(logoType));

  // Logo from hosted asset
  if (isImageAsset(logoImageDesktop) && hasValidLogoType && format === 'desktop') {
  ...
  } else if (isImageAsset(logoImageMobile) && hasValidLogoType && format === 'mobile') {
...
const Logo = props => {
  ...
  const { logoImageDesktop, logoImageMobile, logoType } = config.branding;
